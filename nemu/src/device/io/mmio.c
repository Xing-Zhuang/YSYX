...
/* device interface */
//例：add_mmio_map("serial", CONFIG_SERIAL_MMIO, serial_base, 8, serial_io_handler); 
//设备名字  物理内存中起始地址  IO空间中的起始地址（init_map函数开辟的）  该设备所需的IO空间字节数  回调函数
void add_mmio_map(const char *name, paddr_t addr, void *space, uint32_t len, io_callback_t callback) {
  ...
  Log("Add mmio map '%s' at [" FMT_PADDR ", " FMT_PADDR "]",
      maps[nr_map].name, maps[nr_map].low, maps[nr_map].high);//在控制台打印提示信息
